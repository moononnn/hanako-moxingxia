...
/**
 * 调 Hana 的「重新发现模型」：POST /api/providers/fetch-models。
 * 对自定义 OpenAI 兼容供应商会直接请求其 /v1/models 拉真实模型列表；
 * 内置/OAuth 供应商走 registry 或默认目录。返回 { models, source, error? }。
 */
export async function fetchProviderModels(server, providerId, extra = {}) {
  const body = { name: providerId, ...extra };
  const { ok, status, body: resp } = await apiFetch(server, "/api/providers/fetch-models", {
    method: "POST",
    body: JSON.stringify(body),
  });
  if (!ok) {
    const msg = resp?.error || `HTTP ${status}`;
    throw new Error(`重新发现模型失败：${msg}`);
  }
  if (resp?.error && !Array.isArray(resp.models)) {
    throw new Error(`重新发现模型失败：${resp.error}`);
  }
  return {
    models: Array.isArray(resp?.models) ? resp.models : [],
    source: resp?.source || null,
    error: resp?.error || null,
    ignoredModels: Array.isArray(resp?.ignoredModels) ? resp.ignoredModels : [],
  };
}
...
/** 按 provider id 找 provider-plugins 里的定义文件；找不到返回 null */
export function resolveLocalProviderFile(hanakoHome, providerId) {
  const root = path.join(hanakoHome, PROVIDER_PLUGINS_DIR);
  let entries;
  try {
    entries = fs.readdirSync(root, { withFileTypes: true });
  ...
  for (const entry of entries) {
    ...
    const dir = path.join(root, entry.name);
    const manifest = readJsonObject(path.join(dir, "manifest.json"));
    if (manifest?.provider !== providerId) continue;
    const configPath = path.join(dir, "providers", entry.name + ".json");
    if (!fs.existsSync(configPath)) continue;
    return { storageId: entry.name, path: configPath };
  }
  return null;
}
