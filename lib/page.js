...
(() => {
  ...
  async function refreshFw() {
    ...
    // 状态区
    if (fwStateEl) {
      const cls = data.degraded ? "fw-state degraded" : "fw-state";
      const degradeInfo = data.degraded
        ? "备用模型正在顶上（已降级）" + (data.degradedAt ? "，降级于 " + new Date(data.degradedAt).toLocaleString() : "")
        : (data.enabled ? "保护已开启，备用模型就位" : "尚未配置备用模型，降级保护未启用");
      const cappedTip = (data.thinkingCapped || 0) > 0
        ? '<div class="fw-capped">⚙️ 备用模型是深度思考型，部分小预算调用可能只思考没输出正文（已自动忽略 ' + (data.thinkingCapped || 0) + ' 次），不影响它继续顶上</div>'
        : "";
      fwStateEl.className = cls;
      fwStateEl.innerHTML = '<div>' + esc(degradeInfo) + '</div>'
        + '<div class="fw-stat">'
        + '<span class="fw-stat-item">连续失败 <b>' + (data.consecutiveFailures || 0) + '</b> 次</span>'
        + '<span class="fw-stat-item">触发阈值 <b>2</b> 次</span>'
        + (data.backup ? '<span class="fw-stat-item">备用 <b>' + esc(data.backup.utility || data.backup.vision || "已配") + '</b></span>' : '')
        + '</div>'
        + cappedTip;
    }
    ...
    if (fwEventsEl) {
      ...
      fwEventsEl.innerHTML = evs.length === 0
        ? '<div class="ev">还没有降级事件</div>'
        : evs.map((e) => '<div class="ev">' + esc(e.type) + " · " + esc(new Date(e.at).toLocaleString()) + "</div>").join("");
    }
  ...
  // 加载 Hana 内置模型列表，填充三个下拉框（视觉模型只显示能识图的）
  async function loadFwModels(selected, current) {
    ...
    for (const slot of slots) {
      ...
      // 视觉模型只列能识图的（input 含 image）；工具模型全部列出
      ...
      const emptyText = slot.id === "fw-util" ? "— 不设备用 —" : slot.id === "fw-util-large" ? "— 跟随工具模型 —" : "— 跟随主模型 —";
  ...
  document.querySelectorAll("[data-fw-test]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const label = selId === "fw-util" ? "工具" : selId === "fw-util-large" ? "大工具" : "视觉";
  ...
  if (fwResetBtn) {
    fwResetBtn.addEventListener("click", async () => {
      try {
        ...
        toast(data.restored ? "已手动切回主模型" : "当前没在降级态，不用恢复");
        await refreshFw();
      } catch (err) {
        toast(err.message, true);
      }
...
<body>
  ...
  <div class="fw-panel" id="fw-panel">
    <h2>🛟 降级保护</h2>
    <div class="fw-sub">Hana 的工具模型 / 视觉模型失败时（空响应、超时、限流等），自动切到下面的备用模型临时顶上；很快（约 1 分钟）自动切回你的主模型再试，试不动再切备用。你的模型永远是默认，备用只是救火员。</div>
    <div class="fw-row">
      <label>工具模型</label>
      <div class="fw-pair">
        <select class="fw-select" id="fw-util-provider"><option value="">— 不设备用 —</option></select>
    ...
    <div class="fw-row">
      <label>大工具模型</label>
      <div class="fw-pair">
        <select class="fw-select" id="fw-util-large-provider"><option value="">— 跟随工具模型 —</option></select>
    ...
    <div class="fw-row" style="display:none" data-fw-vision-hidden>
      <label>视觉模型</label>
      <div class="fw-pair">
        <select class="fw-select" id="fw-vision-provider"><option value="">— 跟随主模型 —</option></select>
