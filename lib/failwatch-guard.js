...
export class FailwatchGuard {
  ...
  /**
   * 主模型的思考档位（defaultThinkingLevel），用于「主模型 thinking-capped」兼容判定。
   * 通过 deps.getMainThinkingLevel 注入；读不到返回 null → 保守不兼容，走原判定。
   */
  async _mainThinkingLevel() {
    try {
      const level = await this.deps.getMainThinkingLevel?.();
      return typeof level === "string" && level ? level : null;
    } catch {
      return null;
    }
  ...
  async tick(linesOverride) {
    ...
    // 主模型 thinking-capped 兼容（2026-09-01 实测）：主模型也可能是深度思考型
    // （如 command code 的 deepseek thinking_level=max），滚动摘要小预算下会空响应
    // （「模型未回复正文」），这不是主模型真坏，是「想太多没来得及说」。
    // 此时不累计失败、不降级，只记 thinkingCapped 计数。
    if (!state.degraded) {
      const mainLevel = await this._mainThinkingLevel();
      const mainCapped = mainLevel && failures.some((f) => isThinkingCappedFailure(f, mainLevel));
      if (mainCapped) {
        const prev = store.failwatch || {};
        store.failwatch = {
          ...prev,
        ...
        this._save(store);
        return { action: "main-thinking-capped", failures, reason: "main-thinking-capped", capped: true };
      }
    }
